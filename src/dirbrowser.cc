...
/* modified for Aria by <tujikawa@rednoah.com> */
...
static void ok_clicked(GtkWidget *widget, GtkWidget *tree)
{
    ...
    window = (GtkWidget *)gtk_object_get_user_data(GTK_OBJECT(widget));
...
GtkWidget *create_dir_browser(const gchar *title, const gchar *current_path, GtkSelectionMode mode, void (*handler) (gchar *))
{
    ...
    g_signal_connect(GTK_OBJECT(tree), "tree_expand", GTK_SIGNAL_FUNC(expand_cb), NULL);
    g_signal_connect(GTK_OBJECT(tree), "select_row", GTK_SIGNAL_FUNC(select_row_cb), NULL);
    ...
    g_signal_connect(GTK_OBJECT(ok), "clicked", GTK_SIGNAL_FUNC(ok_clicked), GTK_OBJECT(tree));
    gtk_widget_show(ok);

    cancel = gtk_button_new_with_label(_("Cancel"));
    GTK_WIDGET_SET_FLAGS(cancel, GTK_CAN_DEFAULT);
    gtk_box_pack_start(GTK_BOX(bbox), cancel, TRUE, TRUE, 0);
    g_signal_connect_swapped(GTK_OBJECT(cancel), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(window));
    ...
    if (node_found)gtk_ctree_node_moveto(GTK_CTREE(tree), node,
                                             0, 0.5, 0.5);
