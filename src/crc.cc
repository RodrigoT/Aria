...
bool CRC_check_main(ItemCell *itemcell)
{
  ...
  if(crc_type_specified == ItemCell::CRC_16) {
    crc_length = 4;
  ...
  line = _("CRC is ")+itos_hex(crc, crc_length, '0');
  itemcell->Send_message_to_gui(line, MSG_DOWNLOAD_INFO);

  if(crc_specified == crc) {
    // crc matched
    line = _("CRC matched");
    ...
    // crc error
    //itemcell->set_Status(ITEM_CRCERROR);
    itemcell->Send_status();
    line = _("Invalid CRC. The specified CRC is ")+itos_hex(crc_specified, crc_length, '0');
